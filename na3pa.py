...
def make_PA_Graph(total_nodes, out_degree):
    ...
    PA_graph = nx.complete_graph(out_degree)
    trial = PATrial(out_degree)
    for vertex in range(out_degree, total_nodes):
        for node in trial.run_trial(out_degree):
            PA_graph.add_edge(vertex, node)
    return PA_graph

if __name__ == "__main__":
    PA_graph = make_PA_Graph(20, 5)
    print(PA_graph.edges())
